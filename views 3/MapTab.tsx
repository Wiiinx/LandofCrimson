...
export const MapTab: React.FC<MapTabProps> = ({ currentUser, handleTravel }) => {
  ...
  const [transform, setTransform] = useState({ x: -400, y: -200, scale: 0.6 });
  ...
  const handleWheel = (e: React.WheelEvent) => {
    ...
    const newScale = Math.min(Math.max(0.3, transform.scale - e.deltaY * scaleSensitivity), 3);
  ...
  const handleReset = () => {
    if (currentCityData) {
        ...
        setTransform({ 
            x: -currentCityData.x * 0.8 + containerW / 2, 
            y: -currentCityData.y * 0.8 + containerH / 2, 
            scale: 0.8 
        });
    }
  };
    ...
    <div className="relative w-full h-full overflow-hidden bg-[#0a0a0a] select-none group">
       ...
       >
         <div 
            ...
            className="relative"
         >
            {/* World Background Shape (Abstract) */}
            <div className="absolute top-0 left-0 w-[2560px] h-[1440px] opacity-10 pointer-events-none border border-zinc-800 bg-zinc-900/50"></div>
       ...
       <div className="absolute top-6 left-6 pointer-events-none z-20">
            <h2 className="text-3xl font-black font-orbitron text-white tracking-widest uppercase drop-shadow-md">
                WORLD MAP <span className="text-red-600 text-sm align-top">V.2.0</span>
            </h2>
